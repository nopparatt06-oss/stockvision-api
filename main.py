...
import threading, urllib.request, time as _time
...
def get_price(symbol: str):
    try:
        ...
        return {"symbol": symbol.upper(), "price": price, "prev": prev,
                "change": chg_amt, "changePct": chg_pct, "currency": "USD"}
    ...
    for sym in sym_list:
        try:
            ...
            results[sym] = {"symbol": sym, "price": price,
                            "changePct": chg_pct, "change": round(price - prev, 2)}
        except Exception as e:
...
threading.Thread(target=_self_ping                results[sym] = {"error": "not found"}
                continue
            price = round(float(hist["Close"].iloc[-1]), 2)
            prev  = round(float(hist["Close"].iloc[-2]), 2) if len(hist) >= 2 else price
            chg_pct = round(((price - prev) / prev) * 100, 2) if prev else 0
